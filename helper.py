...
def medal_tally(df):
    ...
    # change data type str into int
    return medal_tally


def country_year_list(df):
    # unique value of Year column
...
def yearwise_medal_tally(df,country):
    temp_df=df.dropna(subset=['Medal'])
    temp_df.drop_duplicates(subset=['Team', 'NOC', 'Games', 'Sport', 'Medal', 'City', 'Event', 'Year'], inplace=True)
    new_df = df[df['region'] == country]
    final_df = new_df.groupby('Year').count()['Medal'].reset_index()
    return final_df


def most_athelets(df, country):
    temp_df = df.dropna(subset=["Medal"])
